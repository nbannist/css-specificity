...
/* 
	...
	> (this is ignored) To calculate a, count 1 if the declaration is from a style attribute rather than a rule with a selector (an inline style), 0 otherwise. (I ignore this)
...
$(document).ready(function() {
    ...
    var css_declarations_re = /\{[\s\S]*?\}/gi; // /{([\s\S]*?)*?}/gi;
    var whitespace_set = /([\s]+?)/gi; 
    var selector_separator = /(\{[\s\S]*?\}|\n)/gi;
	...
	$('.clear-textarea-on-click').click(function(clickEvent) {
		$('#css_raw').val("");
	});
	...
	function splitLines(lines) {
	    for (var l=0; l<lines.length; l++) { // loop through lines
	    	if (lines[l].search(css_declarations_re) >= 0) {
	    		// do nothing
	    	} else {
	...
	function calculateLineSpecificity(line) {
	    if (line != null && line.trim() != "") {
            ...
            */
            ...
            details += '<div><span class="label label-info">IDs:</span><span class="count">['+breakdown[0][0]+']</span> <span class="list">'+breakdown[1][0].toString().replace(/,/gi,", ")+'</span></div>';
            details += '<div><span class="label label-info">Classes:</span><span class="count">['+breakdown[0][1]+']</span> <span class="list">'+breakdown[1][1].toString().replace(/,/gi,", ")+'</span></div>';
            details += '<div><span class="label label-info">Pseudo Classes:</span><span class="count">['+breakdown[0][2]+']</span> <span class="list">'+breakdown[1][2].toString().replace(/,/gi,", ")+'</span></div>';
            details += '<div><span class="label label-info">Attributes:</span><span class="count">['+breakdown[0][3]+']</span> <span class="list">'+breakdown[1][3].toString().replace(/,/gi,", ")+'</span></div>';
            details += '<div><span class="label label-info">Elements:</span><span class="count">['+breakdown[0][4]+']</span> <span class="list">'+breakdown[1][4].toString().replace(/,/gi,", ")+'</span></div>';
            details += '<div><span class="label label-info">Pseudo Elements:</span><span class="count">['+breakdown[0][5]+']</span> <span class="list">'+breakdown[1][5].toString().replace(/,/gi,", ")+'</span></div>';
            details += '</div>';
	...
	function findCss3PseudoElements(selector) {
		...
		if (selector.search(css3PseudoElements_re) == 0) {
			return selector.substring(1).search(css3PseudoElements_re)+1;
		} else {
	...
	function findCss2PseudoElements(selector) {
		...
		if (selector.search(css2PseudoElements_re) == 0) {
			return selector.substring(1).search(css2PseudoElements_re)+1;
		} else {
	...
	function findPseudoClass(selector) {
		...
		if (selector.search(pseudoClass_re) == 0) {
			return selector.substring(1).search(pseudoClass_re)+1;
		} else {
	...
	function findCssClass(selector) {
		...
		if (selector.search(cssClass_re) == 0) {
			return selector.substring(1).search(cssClass_re)+1;
		} else {
	...
	function splitOnPseudoElements(selectorArray) {
		...
		for (var i=0; i<temp.length; i++) { // loop through the array
			...
			var splitSelector = [];
			var maxNumOfClasses = currentSelector.split(':').length;
		    for (var p=0; p<maxNumOfClasses; p++) {
                ...
                if (splitSelector.length > 1) { // at least 2
                    returnValue = returnValue.concat(splitSelector[0]);
                    currentSelector = splitSelector[1];
                    pseudoElementCSS3 = findCss3PseudoElements(currentSelector);
                    pseudoElementCSS2 = findCss2PseudoElements(currentSelector);
                    splitSelector = [];
                } else if (splitSelector.length > 0) { // one
                    returnValue = returnValue.concat(splitSelector[0]);
                    break;
                } else {
		...
		for (var i=0; i<temp.length; i++) { // loop through the array
			...
			for (var p=0; p<maxNumOfClasses; p++) {
                ...
                if (splitSelector.length > 1) { // at least 2
                    returnValue = returnValue.concat(splitSelector[0]);
                    currentSelector = splitSelector[1];
                    pseudoClass = findPseudoClass(currentSelector);
                    splitSelector = [];
                } else if (splitSelector.length > 0) {
                    returnValue = returnValue.concat(splitSelector[0]);
                    break;
                } else {
                    returnValue = returnValue.concat(currentSelector);
		...
		for (var i=0; i<temp.length; i++) { // loop through the array
			...
			for (var p=0; p<maxNumOfClasses; p++) {
				...
				if (splitSelector.length > 1) { // at least 2
					...
					splitSelector = [];
				} else if (splitSelector.length > 0) {  // only 1
					returnValue = returnValue.concat(splitSelector[0]);
					break;
				} else { // nothing returned just use the selector
					returnValue = returnValue.concat(currentSelector);
					break;
				}
			}
	...
	function splitOnCombinators(selector) {
        ...
        if (previousSplit == -1) {
        	returnValue.push(selector); // no splits
        } else if (previousSplit < selector.length-1) {
        	returnValue.push(selector.substring(previousSplit).trim());
        }
        return returnValue;
	}
